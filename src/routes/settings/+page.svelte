...
<div class="relative min-h-screen bg-[#0A0118]">
	<!-- Gradient background -->
	<div
		class="absolute inset-0 bg-[radial-gradient(ellipse_80%_80%_at_50%_-20%,rgba(220,38,38,0.15),rgba(255,255,255,0))]"
	/>
	...
	<div class="relative z-10 max-w-4xl mx-auto px-4 py-6">
		...
		<div class="space-y-3 sm:space-y-4">
			...
			<h1
				class="text-3xl sm:text-4xl md:text-5xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-red-500 via-white to-red-500 animate-gradient"
			>
				Settings
			</h1>
		</div>
		...
		<div class="mt-6 space-y-3">
			{#each settings as setting}
				...
				>
					<div
						class="flex items-center gap-4 p-4 rounded-xl bg-black/40 border border-red-500/20 backdrop-blur-sm"
					>
						...
						<ChevronRight
							class="w-4 h-4 text-white/40 group-active:translate-x-1 transition-transform"
						/>
					</div>
				</button>
			{/each}
