...
export const POST: RequestHandler = async ({ request }) => {
	try {
		...
		console.log('[getRecommendation] Request data:', { query, mediaType, genres, platforms, minRating, count });
		...
		// Security: Validate and limit count parameter
		const validatedCount = Math.min(Math.max(parseInt(count) || 5, 1), 5);
